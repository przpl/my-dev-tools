 ...
 *
 * Detection is a set difference between the normal diff and a whitespace-insensitive one. Binary
 * files, mode-only changes and pure additions or deletions survive both passes (or are not reported
 * as modifications), so they never qualify.
 */
export async function findWhitespaceOnlyChanges(gitRoot: string): Promise<string[]> {
    ...
    const meaningful = new Set(parseNumstatPaths(meaningfulChanges));
    const modified = parseModifiedPaths(nameStatus);

    return parseNumstatPaths(allChanges).filter(file => !meaningful.has(file) && modified.has(file));
}

/**
 * Returns the unstaged files that are safe to stage without review. Currently that is only the
 * whitespace-only changes; further classification rules are meant to be unioned in here.
 */
export async function findAutoStageableFiles(gitRoot: string): Promise<string[]> {
    return findWhitespaceOnlyChanges(gitRoot);
}
